...
class TableTennisEnvV0(BaseV0):
  ...
  DEFAULT_OBS_KEYS = ['pelvis_pos', 'body_qpos', 'body_qvel', 'ball_pos', 'ball_vel',
  ...
  def __init__(self, model_path, obsd_model_path=None, seed=None, **kwargs):
    ...
    super().__init__(model_path=model_handle, obsd_model_path=obsd_model_path,
  ...
  def _setup(self,
    ...
    self.init_paddle_quat = R.from_euler('xyz', np.array(
        [-0.3, 1.57, 0]), degrees=False).as_quat()[[3, 0, 1, 2]]
  ...
  def get_obs_dict(self, sim):
    ...
    obs_dict['palm_pos'] = self.sim.data.site_xpos[self.sim.model.site_name2id(
        'S_grasp')]
  ...
  def get_reward_dict(self, obs_dict):
    ...
    act_mag = np.linalg.norm(
        self.obs_dict['act'], axis=-1)/self.sim.model.na if self.sim.model.na != 0 else 0
    ...
    torso_err = abs(
        self.sim.data.qpos[self.sim.model.jnt_qposadr[self.sim.model.joint_name2id('flex_extension')]])
  ...
  def get_metrics(self, paths, successful_steps=1):
    ...
    effort = -1.0*np.mean([np.mean(p['env_infos']['rwd_dict']['act_reg'])
                          for p in paths])
  ...
  def reset(self, reset_qpos=None, reset_qvel=None, **kwargs):
    ...
    if self.ball_friction_range:
      self.sim.model.geom_friction[self.id_info.ball_gid] = self.np_random.uniform(
  ...
  def cal_ball_qvel(self, ball_qpos):
    ...
    """
    table_upper = [1.35, 0.70,
                   0.785]  # set the position's range on the model's side of the table
  ...
  def _preprocess_spec(self,
    ...
    removed_ids.extend(recursive_immobilize(
