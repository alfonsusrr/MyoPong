...
class TableTennisEnvV0(BaseV0):
  ...
  DEFAULT_OBS_KEYS = [
      'pelvis_pos', 'body_qpos', 'body_qvel', 'ball_pos', 'ball_vel',
      'paddle_pos', "paddle_vel", 'paddle_ori', 'reach_err', "touching_info"
  ]
  ...
  def __init__(self, model_path, obsd_model_path=None, seed=None, **kwargs):
    ...
    gym.utils.EzPickle.__init__(self, model_path, obsd_model_path, seed,
                                **kwargs)
    preproc_kwargs = {
        "remove_body_collisions": kwargs.pop("remove_body_collisions",
                                             True),
    ...
    super().__init__(model_path=model_handle,
  ...
  def _setup(
    ...
    self.init_paddle_quat = R.from_euler(
        'xyz', np.array([-0.3, 1.57,
                         0]), degrees=False).as_quat()[[3, 0, 1, 2]]
    ...
    self.ball_posadr = self.sim.model.joint(
        "pingpong_freejoint").qposadr[0]
    ...
    self.start_vel = np.array([[5.6, 1.6,
                                0.1]])  # np.array([[5.5, 1, -2.8] ])
  ...
  def get_obs_dict(self, sim):
    ...
    obs_dict['body_qpos'] = sim.data.qpos[
        self.id_info.myo_joint_range].copy()
    obs_dict['body_qvel'] = sim.data.qvel[
        self.id_info.myo_dof_range].copy()

    obs_dict["ball_pos"] = sim.data.site_xpos[self.id_info.ball_sid]
    obs_dict["ball_vel"] = self.get_sensor_by_name(sim.model, sim.data,
                                                   "pingpong_vel_sensor")
    ...
    obs_dict[
        'padde_ori_err'] = obs_dict["paddle_ori"] - self.init_paddle_quat
  ...
  def get_reward_dict(self, obs_dict):
    reach_dist = np.abs(np.linalg.norm(self.obs_dict['reach_err'],
                                       axis=-1))
    palm_dist = np.abs(np.linalg.norm(self.obs_dict['palm_err'], axis=-1))
    act_mag = np.linalg.norm(
        self.obs_dict['act'],
        axis=-1) / self.sim.model.na if self.sim.model.na != 0 else 0
    ball_pos = obs_dict["ball_pos"][0][0] if obs_dict[
        'ball_pos'].ndim == 3 else obs_dict['ball_pos']
    ...
    torso_err = abs(self.sim.data.qpos[self.sim.model.jnt_qposadr[
        self.sim.model.joint_name2id('flex_extension')]])
    paddle_touch = obs_dict['touching_info'][0][0] if obs_dict[
        'touching_info'].ndim == 3 else obs_dict['touching_info']
...
"""
  ...
  def _get_done(self, z, solved):
    if self.obs_dict['time'] > MAX_TIME:
      ...
      return 1
    elif evaluate_pingpong_trajectory(
            self.contact_trajectory) in [0, 2, 3]:
      return 1
    return 0
...
"""
    ...
    for path in paths:
      ...
      if np.sum(path['env_infos']['rwd_dict']['solved'] *
                1.0) >= successful_steps:
        num_success += 1
  ...
  def reset(self, reset_qpos=None, reset_qvel=None, **kwargs):
    ...
    if self.paddle_mass_range:
      self.sim.model.body_mass[
          self.id_info.paddle_bid] = self.np_random.uniform(
              *self.paddle_mass_range)

    # friction of the ball changes
    if self.ball_friction_range:
      self.sim.model.geom_friction[
          self.id_info.ball_gid] = self.np_random.uniform(
    ...
    if self.qpos_noise_range is not None:
      joint_ranges = self.sim.model.jnt_range[:,
                                              1] - self.sim.model.jnt_range[:,
                                                                            0]
      noise_fraction = self.np_random.uniform(**self.qpos_noise_range,
                                              size=joint_ranges.shape)
    ...
    obs = super().reset(reset_qpos=reset_qpos_local,
...
"""
  ...
  def relaunch_ball(self):
    ...
    if self.ball_qvel:
      ...
      self.init_qvel[self.ball_dofadr:self.ball_dofadr +
                     3] = ball_vel[:3]
  ...
  def step(self, a, **kwargs):
    ...
    if self.normalize_act:
      ...
      processed_controls[robotic_act_ind] = (
          np.mean(self.sim.model.actuator_ctrlrange[robotic_act_ind],
                  axis=-1) + processed_controls[robotic_act_ind] *
  ...
  def _preprocess_spec(self,
    ...
    removed_ids = recursive_immobilize(spec,
                                       temp_model,
                                       spec.body("femur_l"),
                                       remove_eqs=True)
    removed_ids.extend(
        recursive_immobilize(spec,
                             temp_model,
                             spec.body("femur_r"),
                             remove_eqs=True))
    ...
    if add_left_arm:
      ...
      recursive_remove_contacts(spec_copy.worldbody,
                                return_condition=None)

      meshes_to_mirror = set()

      recursive_mirror(meshes_to_mirror, spec_copy,
                       spec_copy.body("clavicle"))
