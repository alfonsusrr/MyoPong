...
class ParallelCollectWrapper(gym.Wrapper):
  ...
  def _process_obs(self, obs):
    if isinstance(obs, dict) and hasattr(self.env.unwrapped, "obsdict2obsvec"):
      _, obs_vec = self.env.unwrapped.obsdict2obsvec(
          self.env.unwrapped.obs_dict, self.env.unwrapped.obs_keys)
      return obs_vec
    return obs
...
def collect_activations(checkpoint_path: Path, env_id: str, episodes: int, percentile: int, deterministic: bool = False, n_envs: int = 1) -> np.ndarray:
  ...
  vec_env = make_vec_env(make_env(env_id), n_envs=n_envs,
  ...
  print(
      f"Collected data from {success_count} successful episodes (out of {episodes_completed}).")
  return np.array(solved_acts)


def parse_args() -> argparse.Namespace:
  parser = argparse.ArgumentParser(
      description="Collect muscle activations from PPO policy")
  parser.add_argument("--env-id", type=str,
                      default="myoChallengeTableTennisP1-v0", help="Environment ID")
  parser.add_argument("--checkpoint", type=str, required=True,
                      help="Path to PPO checkpoint")
  parser.add_argument("--output", type=str, default="activations.npy",
                      help="Output path for .npy file")
  parser.add_argument("--episodes", type=int, default=2000,
                      help="Number of episodes to run")
  parser.add_argument("--percentile", type=int, default=80,
                      help="Percentile for reward threshold")
