...
def plot_on_ax(ax1, results, title):
    ...
    # Customize axes
    ax1.set_ylabel('Success Rate', fontweight='bold', color='black', labelpad=15, fontsize=28)
    ax2.set_ylabel('Effort', fontweight='bold', color='black', labelpad=15, fontsize=28)
    
    ax1.set_title(title, pad=30, fontweight='bold', fontsize=32)
    ax1.set_xticks(x)
    ax1.set_xticklabels(names, rotation=15, ha='right', fontsize=22)
    ...
    ax1.legend(handles=[score_patch, effort_patch], loc='upper left', frameon=True, fontsize=20)
...
if __name__ == "__main__":
    ...
    fig_combined, (ax_top, ax_bottom) = plt.subplots(2, 1, figsize=(20, 18), dpi=100)
    ...
    # Save combined plot
    output_filename_combined = 'eval_comparison_combined'
    plt.savefig(f"{output_filename_combined}.pdf", bbox_inches='tight')
    plt.savefig(f"{output_filename_combined}.png", bbox_inches='tight', dpi=300)
    print(f"Saved combined figures to {output_filename_combined}.pdf and {output_filename_combined}.png")
    plt.close(fig_combined)
    
    # Create separate Phase 1 figure
    fig_p1, ax_p1 = plt.subplots(1, 1, figsize=(18, 12), dpi=100)
    plot_on_ax(ax_p1, results_p1, "Phase 1 Evaluation")
    plt.tight_layout(pad=2.0)
    output_filename_p1 = 'phase_1_eval_comparison'
    plt.savefig(f"{output_filename_p1}.pdf", bbox_inches='tight')
    plt.savefig(f"{output_filename_p1}.png", bbox_inches='tight', dpi=300)
    print(f"Saved Phase 1 figures to {output_filename_p1}.pdf and {output_filename_p1}.png")
    plt.close(fig_p1)
    
    # Create separate Phase 2 figure
    fig_p2, ax_p2 = plt.subplots(1, 1, figsize=(18, 12), dpi=100)
    plot_on_ax(ax_p2, results_p2, "Phase 2 Evaluation")
    plt.tight_layout(pad=2.0)
    output_filename_p2 = 'phase_2_eval_comparison'
    plt.savefig(f"{output_filename_p2}.pdf", bbox_inches='tight')
    plt.savefig(f"{output_filename_p2}.png", bbox_inches='tight', dpi=300)
    print(f"Saved Phase 2 figures to {output_filename_p2}.pdf and {output_filename_p2}.png")
    plt.close(fig_p2)
