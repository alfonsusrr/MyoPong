...
class ActionSpaceWrapper(gym.ActionWrapper):
  ...
  def __init__(self, env):
    ...
    self.action_mapping: Dict[int, List[int]] = {
        ...
        # IO right (note: label per source)
    ...
    if (self.num_timesteps - self._last_eval_step) >= self.eval_freq:
      ...
      try:
        ...
        if metrics:
          ...
          if self.verbose:
            print(
                f"[Eval] step={self.num_timesteps} metrics={metrics} duration_s={duration_s:.2f}")
...
class CurriculumManager(BaseCallback):
  ...
  def _on_step(self) -> bool:
    ...
    if (self.num_timesteps - self._last_eval_step) >= self.eval_freq:
      ...
      try:
        ...
        duration_s = time.time() - t0
        metric_val = float(metrics.get(self.metric_key, 0.0)
        ...
        if self._passes >= self.patience and self.level < (self.steps - 1):
          ...
          if self.verbose:
            print(
                f"[Curriculum] Promoted to level {self.level} at step {self.num_timesteps}")
      except Exception as e:
        if self.verbose:
  ...
  def _apply_current_level(self) -> None:
    ...
    try:
      ...
      self.training_vec_env.env_method(
          'apply_curriculum', self.callable_name, float(value), indices=None)
...
def parse_args() -> argparse.Namespace:
  parser = argparse.ArgumentParser(
      description="Train PPO on MyoSuite with reduced action space and W&B logging")
  ...
  parser.add_argument('--log-dir', type=str, required=True,
                      help='Directory for SB3 logs')
  parser.add_argument('--video-dir', type=str, required=True,
                      help='Directory to save training videos')
  ...
  parser.add_argument('--no-subproc', action='store_true',
                      help='Disable subprocess vectorized envs even if num-envs>1')
  parser.add_argument('--checkpoint-freq', type=int, default=50000,
                      help='Save checkpoint every N steps')
  parser.add_argument('--video-freq', type=int, default=50000,
                      help='Record rollout video every N steps')
  parser.add_argument('--video-rollout-steps', type=int, default=600,
                      help='Steps to render in each video')
  ...
  parser.add_argument('--curriculum-enable', action='store_true',
                      help='Enable curriculum learning during training')
  parser.add_argument('--curriculum-callable', type=str, default=None,
                      help='Env method to set difficulty (e.g., set_difficulty) or use tabletennis_auto')
  parser.add_argument('--curriculum-start', type=float, default=0.1,
                      help='Starting difficulty value passed to curriculum callable')
  parser.add_argument('--curriculum-end', type=float, default=1.0,
                      help='Final difficulty value passed to curriculum callable')
  parser.add_argument('--curriculum-steps', type=int, default=5,
                      help='Number of curriculum promotion steps (levels)')
  parser.add_argument('--curriculum-metric', type=str, default='score',
                      help='Evaluation metric key to gate promotions (from env.get_metrics)')
  parser.add_argument('--curriculum-threshold', type=float, default=0.7,
                      help='Threshold on metric to promote to next level')
  parser.add_argument('--curriculum-patience', type=int, default=2,
                      help='Consecutive passes required before promotion')
  parser.add_argument('--curriculum-eval-freq', type=int, default=20000,
                      help='How often (steps) to run curriculum evaluation')
  parser.add_argument('--curriculum-eval-envs', type=int, default=2,
                      help='Number of envs for curriculum evaluation')
  parser.add_argument('--curriculum-eval-episodes', type=int,
                      default=10, help='Episodes per curriculum evaluation')
...
def main() -> None:
  ...
  vec_env = build_vec_env(
      args.env_id, args.num_envs if not args.no_subproc else 1, args.seed)
  ...
  callbacks.append(PeriodicEvaluator(env_id=args.env_id, eval_freq=args.eval_freq,
                   eval_envs=args.eval_envs, eval_episodes=args.eval_episodes, seed=args.seed, verbose=1))
  ...
  model.learn(total_timesteps=args.total_timesteps, callback=callbacks,
              progress_bar=True, reset_num_timesteps=not args.resume_from_checkpoint)
