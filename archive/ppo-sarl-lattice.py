...
def parse_args() -> argparse.Namespace:
  ...
  parser.add_argument("--wandb-project", type=str, default="myosuite-ppo-sarl-lattice",
  ...
  parser.add_argument("--std-clip-min", type=float, default=1e-3,
                      help="Minimum value for std clipping in lattice policy")
  parser.add_argument("--std-clip-max", type=float, default=10.0,
                      help="Maximum value for std clipping in lattice policy")
  parser.add_argument("--expln-eps", type=float, default=1e-6,
                      help="Epsilon for expln function in lattice policy")
  parser.add_argument("--std-reg", type=float, default=0.0,
                      help="Standard deviation regularization in lattice policy")
  parser.add_argument("--lattice-alpha", type=float, default=1.0,
                      help="Relative weight between action and latent noise in lattice policy")
...
def main() -> None:
  ...
  run_id = f"run-ppo-sarl-lattice-{time.strftime('%Y%m%d-%H%M%S')}-lvl{args.difficulty}"
  ...
  if args.resume_from_checkpoint:
    ...
    model = PPO(
        ...
        policy_kwargs=dict(
            # Lattice policy kwargs
            use_lattice=False,
            std_clip=(args.std_clip_min, args.std_clip_max),
            # expln_eps=args.expln_eps,
